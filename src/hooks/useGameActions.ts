// Core game action callbacks shared between keyboard (useInput) and touch (TouchControls).
// No state owned here — all calls route through worldEngine.
...
export function useGameActions({ onRefresh, onOpenTerminal, onOpenElevator }: GameActionOptions): GameActions {
  ...
  return { tryMove, tryInteract, tryChangeFloor };
}
